...
@Injectable()
export class EthUsdcCrawler extends BaseTokenErc20Crawler {
  constructor(
    @InjectRedis()
    redis: Redis,
    ...
    logger.setContext(EthUsdcCrawler.name);

    const crawlerConfig = new CrawlerTokenErc20Config(
      'src/crawler/abis/usdc.json',
      config.ethereum.usdcStartBlock,
      config.ethereum.usdcAddress,
      config.ethereum.decimals,
      config.ethereum.usdcDecimals,
      config.ethereum.usdcIncrementBlock,
